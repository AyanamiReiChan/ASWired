...
<section class="full space-top"><h3>逐节点限额覆盖</h3><p class="hint">留空继承，0 表示不限。同一作用域节点优先于服务器；成员覆盖优先于套餐。IP 数量表示同时在线 IP，连接数单独计算。</p>
 <div class="actions"><label class="field grow">选择服务器或节点<AppSelect aria-label="选择服务器或节点" bind:value={selected} options={[{value:'',label:'选择资源'},...available.map(item=>({value:item.id,label:item.name}))]}/></label><button type="button" class="button" disabled={!selected} onclick={()=>{row.nodeLimits={...row.nodeLimits,[selected]:{}};selected='';}}>添加覆盖</button></div>
