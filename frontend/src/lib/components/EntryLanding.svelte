<script lang="ts">
 ...
 async function enter(){try{await api('/api/entry',{method:'POST',body:JSON.stringify({key})});key='';await initialize();await goto('/probe',{replaceState:true});}catch{key='';failed=true;}}
</script>
