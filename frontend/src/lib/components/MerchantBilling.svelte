<script lang="ts">
 ...
 let config=$state({source:'system',direction:'sum',resetDay:1,timezone:'Asia/Shanghai',limitGB:0,revision:''});
...
<Modal bind:open title="商家账期与流量" description="独立记录服务器用量。所有 GB 均按 1,000,000,000 字节计算。" wide>
 <label class="field">服务器<select bind:value={serverId} onchange={()=>{snapshot=null;usedGB=undefined;reason='';load();}} disabled={busy||loading}>{#each demo.data.servers as server}<option value={server.id}>{server.name}</option>{/each}</select></label>
 {#if loading}<p class="hint" role="status">正在读取账期…</p>{/if}
 <div class="form-grid space-top">
  <label class="field">统计来源<select bind:value={config.source} disabled={busy||loading}><option value="system">系统探针累计流量</option><option value="xray">Xray 原始用户流量（不含倍率）</option></select></label>
  <label class="field">商家计费方向<select bind:value={config.direction} disabled={busy||loading}>{#each Object.entries(directions) as [key,label]}<option value={key}>{label}</option>{/each}</select></label>
 ...
 <p class="hint">按所选时区 00:00 重置；当月没有该日期则按月末。上传、下载均以服务器视角计算。系统来源跟随服务器所选探针。</p>
