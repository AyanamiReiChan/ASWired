...
<section class="workbench" aria-label="服务器出站管理">
 ...
 {#if kind==='node'}<label class="field">出站标签<input bind:value={tag}/></label><label class="field">从节点库选择<select aria-label="从节点库选择" bind:value={nodeId} onchange={chooseNode}><option value="">选择节点</option>{#each demo.data.nodes??[] as row}<option value={row.id}>{row.name}</option>{/each}</select></label>{#if subscriptions.length}<label class="field">使用套餐凭据<select bind:value={subscriptionId}>{#each subscriptions as sub}<option value={sub.id}>{sub.name??sub.id}</option>{/each}</select></label><button class="button small" onclick={useSubscription}>载入连接配置</button>{/if}<label class="field">或粘贴 VLESS TCP REALITY 链接<textarea aria-label="出站节点链接" bind:value={nodeURI} placeholder="vless://…"></textarea></label><button class="button small" onclick={useURI}>转换为出站</button>
 {:else}
